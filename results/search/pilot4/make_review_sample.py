"""Draw the Chunk 3 Round 4 hand-review sample (tightened question; same draw rules as Round 3: 5 calls per gene; Tnf has only 1 no, so 4 yes + 1 no) from the canonical ledger + verdicts.csv.
...
RUN = ROOT / "results/search/pilot4/named_stim_n25"
...
out = ["# Chunk 3 hand-review sample (Round 4, canonical ledger)", "",
...
for g, ny in PLAN.items():
    ...
    for r in pick:
        ...
        out.append(f"| {r['rank']} | {r['paper_id']} | {r['title'][:80]} | **{r["verdict"]}** | {"yes" if x["downgraded"] else x["answer"]} | {ev[:300]} | {x['evidence_names'] or '-'} / {x['evidence_stimulus'] or '-'} | {x['downgraded'] or 'none'} | |")
    out.append("")
R3 = ROOT / "results/search/pilot3/named_strict_n25"
out += ["## Optional: every verdict that changed from Round 3 to Round 4", "",
...
for g in ["Tnf", "Nos2", "Edn1", "Slc7a2", "Tnfsf15", "Hdc", "Tarm1", "Calhm6", "Rnd1", "Lipg", "Col27a1", "Hcar2"]:
    ...
    for p in sorted(set(a) & set(b), key=lambda p: int(b[p]["rank"])):
        if "success" == a[p]["judge_status"] == b[p]["judge_status"] and a[p]["answer"] != b[p]["answer"]:
            why = b[p]["downgraded"] or "model answer changed"
            ev = (b[p]["evidence"] or "(none)").replace("|", "/").replace("\n", " ")[:250]
            out.append(f"| {g} | {b[p]['rank']} | {p} | {b[p]['title'][:70]} | {a[p]['answer']} → **{b[p]['answer']}** | {why} | {ev} | |")
out.append("")
(ROOT / "results/search/pilot4/review_sample.md").write_text("\n".join(out))
